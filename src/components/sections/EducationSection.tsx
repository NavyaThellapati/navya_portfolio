        ...
        >
          ...
          <div className="mt-5 grid gap-2 text-sm text-[var(--muted)]">
            {educationFocus.slice(0, 5).map((focus) => (
              <span key={focus} className="before:mr-2 before:text-[#F08AB8] before:content-['•']">{focus}</span>
            ))}
          </div>
        </motion.div>
        </div>
        ...
        <div className="mt-6 grid gap-3">
          {certifications.map((cert, index) => {
            ...
            return (
              <motion.article
                key={cert.title}
                className="group flex items-center gap-3 rounded-[14px] border border-[var(--line)] bg-[var(--surface)] p-4 transition hover:-translate-y-0.5 hover:border-[#F08AB8]/45 hover:bg-[var(--surface-strong)]"
                initial={{ opacity: 0, y: 22 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: index * 0.06 }}
              >
                <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[#FFF9FF]/8 text-[#C4A7FF]"><Icon className="h-4 w-4" /></span>
