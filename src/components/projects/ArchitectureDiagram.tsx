    ...
    <div className="rounded-3xl border border-[var(--line)] bg-[var(--surface)] p-5">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {nodes.map((node, index) => (
          <div key={node} className="relative rounded-2xl border border-[#C58E9D]/24 bg-[#C58E9D]/10 p-4">
            <p className="text-xs font-bold uppercase tracking-[0.24em] text-[#F6D8DF]">0{index + 1}</p>
            <p className="mt-3 font-display text-lg font-bold text-[var(--text)]">{node}</p>
            {index < nodes.length - 1 ? <span className="absolute -right-3 top-1/2 hidden h-px w-6 bg-[#C9A86A]/50 lg:block" /> : null}
          </div>
        ))}
      </div>
    </div>
